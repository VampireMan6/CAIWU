<template>
	<view class="serach f-between f-center">
		<img class="returnimg" src="../../static/tab-1//return-b.png" alt="" @click="returns">
		<view class="content">
			<!-- HM修改 增加进入输入状态的点击范围 -->
			<view class="content-box" :class="{'center':mode === 2}" @click="getFocus">
				...
				<text v-show="!shows" class="icon icon-del"  @click="clear"></text>
			</view>
	...
	<!-- 	<view  v-if="button === 'outside'" class="button" :class="{'active':show||active}" @click="search">
			<view class="button-item" v-show="show == true">{{searchName}}</view>
			<view class="button-item" v-show="show == false">搜索</view>
		</view> -->
	</view>
</template>
...
.serach {
	...
	.content {
		...
		width: 600upx;
		height: 60upx;
		// border: 2upx #F8F8F8 solid; //HM修改 去掉边框
		background: #F8F8F8;
		overflow: hidden;
		border-radius: 30upx;
